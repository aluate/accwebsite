              ...
              <div key={room.id} className="bg-[#2d2d2d] rounded p-5">
                ...
                {room.accessories.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-white/10">
                    <p className="text-white/30 text-[10px] font-condensed uppercase tracking-widest mb-2">Accessories</p>
                    {room.accessories.map((acc, i) => {
                      const a = catalogs.revaAccessories.find((x) => x.id === acc.acc_id);
                      return a ? <p key={i} className="text-white/60 text-xs">{acc.qty}x {a.name}</p> : null;
                    })}
                  </div>
                )}
                {room.notes && <p className="text-white/40 text-xs mt-3 italic">{room.notes}</p>}
              </div>
            );
          })}
          {rooms.length > 0 && dirty && (
            <p className="text-yellow-400/60 text-xs font-condensed uppercase tracking-widest">Save before continuing.</p>
          )}
        </div>
      )}
      <input type="hidden" value={specId} readOnly />
    </div>
  );
}

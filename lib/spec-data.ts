...
type FGRow = { id: string; label: string; finish_type: string; notes: string | null; species: string | null; color_id: string | null; color_name: string | null; door_style_id: string | null; pull_id: string | null; carcass_id: string | null; drawer_box_id: string | null; rollout_box_id: string | null; edgeband_id: string | null; applied_panels: string | null; sort_order: number; wo_number: string | null; ct_material: string | null; ct_style: string | null; ct_edge: string | null; ct_splash: string | null; grain_orientation: string | null; };
...
export async function loadSpecPDFData(specId: string): Promise<SpecPDFData> {
  ...
  */
  ...
  return {
    // job_id is for DISPLAY (footers). job_internal_id is the key — use that for
    // foreign keys, storage paths and lookups. See the note on SpecPDFData.
    job_id: job.job_number ?? spec.job_id, job_internal_id: spec.job_id, job_number: job.job_number ?? null, engineer: job.engineer ?? null, spec_name: spec.name,

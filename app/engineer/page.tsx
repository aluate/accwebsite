...
export default async function EngineerPage() {
  ...
  const specs = await withDbTimeout((signal) =>
    sql({ signal })<SpecRow[]>`
      ...
      ORDER BY
        CASE rs.lifecycle_state WHEN 'RELEASED_TO_ENG' THEN 0 WHEN 'PM_REVIEW' THEN 1 WHEN 'ENGINEERED' THEN 2 ELSE 3 END,
        rs.updated_at DESC
    `,
  );
    ...
    <section className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-white/30 text-xs font-condensed uppercase tracking-widest mt-1">{session.name} · {queue.length} waiting</p>
        </div>

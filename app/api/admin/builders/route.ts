...
type AccountRow = {
  ...
  active: boolean; created_at: string; role: string;
};

// GET /api/admin/builders — list all accounts (incl. role)
export async function GET() {
  const rows = await sql<AccountRow[]>`
    SELECT id, username, name, company, email, phone, active, created_at, role
...
//   body: { id, active?, password?, name?, company?, email?, phone?, role? }
export async function PATCH(req: NextRequest) {
  const { id, active, password, name, company, email, phone, role } = await req.json();
  ...
  if (password) {
    const hash = await bcrypt.hash(password, 12);
    await sql`UPDATE builder_accounts SET password_hash = ${hash} WHERE id = ${id}`;
  }
  ...
  if (phone !== undefined)   await sql`UPDATE builder_accounts SET phone = ${phone ?? null} WHERE id = ${id}`;

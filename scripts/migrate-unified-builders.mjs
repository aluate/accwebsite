...
`);
// Add new columns to existing tables in case it already existed without them
await sql.unsafe(`ALTER TABLE builders ADD COLUMN IF NOT EXISTS default_finish_type            TEXT DEFAULT 'paint'`);
await sql.unsafe(`ALTER TABLE builders ADD COLUMN IF NOT EXISTS default_carcass_id             TEXT`);
await sql.unsafe(`ALTER TABLE builders ADD COLUMN IF NOT EXISTS default_drawer_box_id          TEXT`);
await sql.unsafe(`ALTER TABLE builders ADD COLUMN IF NOT EXISTS default_pull_id               TEXT`);
await sql.unsafe(`ALTER TABLE builders ADD COLUMN IF NOT EXISTS default_paint_brand           TEXT`);
await sql.unsafe(`ALTER TABLE builders ADD COLUMN IF NOT EXISTS default_accessories           TEXT`);
await sql.unsafe(`ALTER TABLE builders ADD COLUMN IF NOT EXISTS preferred_cabdoor_usage_groups TEXT`);
await sql.unsafe(`ALTER TABLE builders ADD COLUMN IF NOT EXISTS is_residential_default        INTEGER DEFAULT 0`);
console.log("  Done.");
...
console.log("\nAll done! Run the app and go to Admin → Builder Companies to verify.");
await sql.end();

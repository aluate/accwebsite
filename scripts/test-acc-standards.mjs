...
const { seedAccStandards, ACC_STANDARD_HINGE, ACC_STANDARD_DRAWER_SLIDE, ACC_STANDARD_ROLLOUT_SLIDE } =
  await import("../lib/acc-standards-seed.ts");
...
async function t1_freshGroup() {
  ...
  check("drawer_box row carries slides_id", roleOf(d, "drawer_box")?.slides_id === ACC_STANDARD_DRAWER_SLIDE, JSON.stringify(d));
...
async function t3_neverClobber() {
  ...
  await sql`INSERT INTO finish_group_drawers (id, finish_group_id, role, drawer_box_id, slides_id, sort_order)
            VALUES (${uid()}, ${fg}, 'drawer_box', 'DB-CUSTOM', 'HDS-SAL-001', 0)`;
  ...
  check("non-standard slides on the drawer row kept", roleOf(d, "drawer_box").slides_id === "HDS-SAL-001");
...
async function t4_partialBackfill() {
  ...
  await seedAccStandards(fg, { drawerBoxId: "DB-001" });
  const d = await dr(fg);
  check("slides_id backfilled on the existing row", roleOf(d, "drawer_box").slides_id === ACC_STANDARD_DRAWER_SLIDE, JSON.stringify(d));
...
async function t5_rollouts() {
  ...
  check("rollout drawer row created with those slides", roleOf(d, "rollout")?.slides_id === ACC_STANDARD_ROLLOUT_SLIDE);
